...
sap.ui.define([
	...
	"zprs/wipeditor/model/grouper",
	"zprs/wipeditor/model/GroupSortState",
	...
	ReportModel, LineItemsServices, MessageBox) {
	"use strict";

	return BaseController.extend("zprs.wipeditor.controller.Master", {
		...
		onInit: function() {
			...
			this.bus.subscribe("detailToMaster", "toMaster", this.detailToMaster, this);
		...
		onOpenViewSettings: function() {
			if (!this._oViewSettingsDialog) {
				this._oViewSettingsDialog = sap.ui.xmlfragment("zprs.wipeditor.view.ViewSettingsDialog", this);
		...
		detailToMaster: function(detailToMaster, toMaster, data) {
			...
			oModel.read("/WipMattersSet", {
				...
				success: function(oData) {
					if (oData.results.length) {
						...
						oData.results.map(v => v.hcolor = "None");
						that.masterItems = oData.results;
			...
			this.smartfilterId = data.smartfilterid;
			this.bus = sap.ui.getCore().getEventBus();
		...
		onPress: function(oEvent) {
			debugger;
			var pspid = oEvent.getSource().getTitle();
				var listItems = this.masterItems;
			for (var i = 0; i < listItems.length; i++) {
				if (listItems[i].Pspid === pspid) {
					...
					this.jsonModel.setProperty("/Inputs/masterItems", listItems);

			// var InputFields = this.getView().getModel("InputsModel");

			// if (oEvent) {
			// 	InputFields.setProperty("/Inputs/onPressEvent", oEvent.getSource().getBindingContext("InputsModel").getObject());
			// 	Pspid = oEvent.getSource().getProperty("title");
			...
			// this.filterArr = [];
			// var selData = InputFields.getProperty("/Inputs/onPressEvent");

			// this.getView().getModel("InputsModel").setProperty("/Inputs/selectedMatter", selData);
			// var matterInfo = this.getView().getModel("InputsModel").getProperty("/Inputs/selectedMatter");
			// this.getView().getModel("InputsModel").setProperty("/Inputs/MatterNumber", "Matter Number" + '\xa0' + ":" + '\xa0');
			// this.getView().getModel("InputsModel").setProperty("/Inputs/LeadPartner", "Lead Partner" + '\xa0' + ":" + '\xa0');
			// this.getView().getModel("InputsModel").setProperty("/Inputs/BillingOffice", "Billing Office");

			// this.getView().getModel("InputsModel").setProperty("/Inputs/formMatter", matterInfo.Pspid);
			// this.getView().getModel("InputsModel").setProperty("/Inputs/formLeadPartner", matterInfo.BpatnerName);
			...
			var reqFilters = [];
			var requiredFields = ["Budat", "Belnr", "Pernr"];
			var smartFilterBar = this.smartfilterId;
			if (smartFilterBar.getFilters()[0].hasOwnProperty("aFilters")) {
				for (var filter = 0; filter < smartFilterBar.getFilters()[0].aFilters.length; filter++) {

					var filterdata = smartFilterBar.getFilters()[0].aFilters[filter];
					if (filterdata.hasOwnProperty("aFilters")) {
						if (filterdata.aFilters[0].hasOwnProperty("sPath")) {

							if (requiredFields.includes(filterdata.aFilters[0].sPath)) {
								reqFilters.push(filterdata);
							}
						}
				...
				reqFilters.push(smartFilterBar.getFilters()[0]);
			}
			...
			// if (reqFilters[1]) {
			// 	reqFilters[1].aFilters.forEach(function(item) {

			// 		item.sOperator = "EQ";

			// 	});
			// }
			reqFilters.push(new Filter("Pspid", FilterOperator.EQ, Pspid));

			this.bus.publish("masterToDetail", "toDetail", {
				par1: reqFilters,
				pspid: selData.Pspid,
				BpatnerName: selData.BpatnerName,
				BofficeName: selData.BofficeName,
				bOffice: selData.Boffice,
				rootPspid: Pspid
			});
			...
			var oTitle = oEvent.getParameter("listItem").getTitle();
				var InputFields = this.getView().getModel("InputsModel");
				var mster = InputFields.getProperty("/Inputs/matserSearchItems");
			for(var i=0;i<mster.length;i++){
				if(oTitle === mster[i].Pspid){
					mster.splice(i,1);
					InputFields.setProperty("/Inputs/matserSearchItems",mster);
				}
			}

		
			var oMatter =InputFields.getProperty("/Inputs/rootPspid");
			...
			if (oTitle === oMatter) {
				...
				this.icontabId.setVisible(false);
					// this.byId("idIconTabBarMulti").getItems()[1].setVisible(true);
					// 	this.byId("idIconTabBarMulti").getItems()[2].setVisible(true);
					// 	this.byId("idIconTabBarMulti").getItems()[3].setVisible(true);
			}
